                ...
                if (isa<GlobalVariable>(op0) && isa<Constant>(op0))
                {
                    ...
                    while ((format = val.find("%s")) != StringRef::npos) {
                        ...
                        if (s0.size()>0) {
                            ...
                            errs() << "Part 1 has " << argOffset << " args\n";
                            ArrayRef<Value*> args0_a(args0);
                            errs() << "Created " <<  *CallInst::Create(I.getCalledFunction(), args0_a, "", &I) << "\n";
                        }
                        std::vector<Value*> args1;
                        args1.push_back(*(opiter++));
                        ArrayRef<Value*> args1_a(args1);
                        errs() << "Created " << *CallInst::Create(I.getCalledFunction(), args1_a, "", &I) << "\n";
                    ...
                    if (found && val.size()>0) {
                        ...
                        errs() << "Created " << *CallInst::Create(I.getCalledFunction(), args_a, "", &I) << "\n";
